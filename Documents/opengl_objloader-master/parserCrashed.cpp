...
struct texture_control texc[]={
    "data/bubble_color.ppm",1,GL_RGB,NULL_A,GL_TEXTURE0,GL_MODULATE,
    "data/fieldstoneN.ppm",2,GL_RGB,NULL_A,GL_TEXTURE1,GL_MODULATE,
    "data/maple.ppm",3,GL_RGBA,wowalpha,GL_TEXTURE2,GL_DECAL,
};
...
float mytexcoords[4][2]={{0.0,1.0},{10.,1.0},{1.0,0.0},{0.0,0.0}};
...
glBegin(GL_QUADS);
    for(int i=0;i<(faceSize);i++){
        ...
        //tcp=texc;
        //while(tcp->name){
           //float mytexcoords[1]={tc[i].x};
          //  glMultiTexCoord2fv(tcp->texunit,mytexcoords[0]);
            //tcp++;
        //}
        //}
...
glBindTexture(GL_TEXTURE_2D,tcp->texid);
glTexImage2D(GL_TEXTURE_2D,0,tcp->format,im_width,im_height,0,tcp->format, 
	GL_UNSIGNED_BYTE,tb_final);
//glTexParameterf(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_NEAREST);
//glTexParameterf(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_NEAREST);
...
float light1_specular[] = { 0.0, 0.0, 1.0, 0.0 };
